...
bool Flight::addTicket(const Ticket &ticket) {
    return ticket.getFlight()->getEconomyCapacity() <= this->getEconomyCapacity() &&
        ticket.getFlight()->getBusinessCapacity() <= this->getBusinessCapacity();

}

bool Flight::operator<(const Flight &rhs) const {
    int l = 0;
    while (l < this->flightCode.length())
    {
        if (this->flightCode.at(l) < rhs.flightCode.at(l))
            return true;
        else if (this->flightCode.at(l) == rhs.flightCode.at(l))
            l++;
        else
            return false;
    }
